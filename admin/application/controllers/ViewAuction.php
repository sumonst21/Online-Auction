...
class ViewAuction extends CI_Controller
{
	...
	//28 Feb 2018 making changes for admin panel
	function allIn($id){
		...
		$data['lotlist'] = $this->Common_model->getAll("tb_lot",array("auction_id"=>$id,"activate"=>'1'))->result_array();
            ...
            <thead>
              <tr>
                ...
                <th style="text-align: center;">Bid Manager</th>
              </tr>
            </thead>
            <tbody>
                <?php 
  ...
  $('.bidFunctn').click(function(){
    ...
    $('#myModal2').modal('show');
                ...
                <tr>
                  ...
                  <td style="text-align: center;"><button id=" <?php echo $list['id_lot']; ?>" class="functn" ><?php echo $list['lot_increment_by']; ?></button></td>
                  
                  
                    <td style="text-align: center;"><button id=" <?php echo $list['id_lot']; ?>" class="bidFunctn" >Bid</button></td>
	...
	function allInData(){
		...
		$fin=array("bidPrice"=>$bidprice,"strbid"=>$buyerstr);
	...
	function getOnline(){
		$users=$this->Common_model->getOnlineUsers()->result_array();
			foreach ($users as $val) {
				if($val['loggedin']!=""){ ?>
					
					<li><a href="#"> <img height="10px" src="<?php base_url(); ?>/admin/assets/green.png"> <?php echo $val['username']; ?><br>
					<span style="color: #808080; font-size: 10px; text-align: center;"><?php echo $val['lastUpdate']; ?></span>
				</a>
	...
	function getLotData()
	{
		...
		if($lotData['lot_start_date']==""){
		    $lotData['lot_start_date']=date("Y-m-d");
		}
		if($lotData['lot_end_date']==""){
		    $lotData['lot_end_date']=date("Y-m-d");
		}
		if($lotData['lot_start_time']==""){
		    $lotData['lot_start_time']=date("H:i:s");
		}
		if($lotData['lot_end_time']==""){
		    $lotData['lot_end_time']=date("H:i:s");
		}
	...
	function viwewlots($id)
	{
			...
			$data['deleteBid']	= base_url().'index.php/ViewAuction/deleteBid';
			...
			$data['addBid']= base_url()."index.php/ViewAuction/addBid";
	...
	function addBid(){
		...
		$latestbuyer = $this->db->query("select buyer_id from tb_bidding where lot_id='".$data['id_lot']."' order by id_bidding DESC")->row_array();
		if($latestbuyer['buyer_id'] != $data['buyer_id']){
			$bidding_data=$this->Common_model->getAll("tb_bidding", array('lot_id' => $data['id_lot'],'buyer_id'=>$data['buyer_id']),"DESC","id_bidding","1")->row_array();
			if($bidding_data['bidding_price']==$data['bidding_price']){
				return;
			}else{
				$id= $this->Common_model->insert("tb_bidding",array("lot_id"=>$data['id_lot'],"buyer_id"=>$data['buyer_id'],"bidding_price"=>$data['bidding_price'],"bid_time"=>$data['bid_time']));
				$info= $this->Common_model->getAll("tb_lot",array("id_lot"=>$data['id_lot']))->row_array();
				$bid = strtotime(date($info['lot_end_date']." ".$info['lot_end_time']));
				$now = strtotime(date('Y-m-d H:i:s'));
				$diff = $bid - $now;
				if($diff < 120){
					$add = 130 - $diff;
					$this->db->query("UPDATE tb_lot SET lot_end_time=(lot_end_time + INTERVAL ".$add." SECOND) WHERE id_lot = ".$data['id_lot']);
				}
			}
			//if($id > 0){ echo "success"; }else{ echo "failed"; }
			$url="Location:".base_url()."index.php/ViewAuction/viwewlots/";
			$lotid=$data['id_lot'];
			$url.=$lotid;
			//echo preg_replace('/\s+/', '', $url);
			header("Location:".preg_replace('/\s+/', '', $url));
			//instaheader("Refresh: 1");
		}
		
		
	}//addBid function ends
